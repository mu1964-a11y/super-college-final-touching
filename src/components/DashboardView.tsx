...
import { 
  Sparkles, BrainCircuit, Mic, Camera, Smartphone, Lock, Key, 
  RefreshCw, AlertTriangle, UserCheck, ShieldAlert, Volume2, 
  Bot, Check, ExternalLink
} from "lucide-react";
...
export default function DashboardView({
  ...
  const [activeDashboardTab, setActiveDashboardTab] = React.useState<"overview" | "directors" | "staff" | "ai_copilot" | "ai_executive">("overview");

  // Executive Bot Live Dashboard States
  const [delegatedAdmins, setDelegatedAdmins] = React.useState<any[]>([]);
  const [botAuditLogs, setBotAuditLogs] = React.useState<any[]>([]);
  const [isLoadingBotData, setIsLoadingBotData] = React.useState<boolean>(false);
  const [previewModalFace, setPreviewModalFace] = React.useState<{ isOpen: boolean; name: string; url: string; phone?: string } | null>(null);
  const [selectedAuditLog, setSelectedAuditLog] = React.useState<any | null>(null);
  const [overviewAuditFilter, setOverviewAuditFilter] = React.useState<"all" | "voice" | "image" | "admin">("all");

  const fetchBotDashboardData = React.useCallback(async () => {
    setIsLoadingBotData(true);
    try {
      const [adminsRes, logsRes] = await Promise.all([
        fetch("/api/whatsapp/delegated-admins").then((r) => r.json()).catch(() => ({ admins: [] })),
        fetch("/api/whatsapp/audit-logs?limit=40").then((r) => r.json()).catch(() => ({ logs: [] })),
      ]);
      if (adminsRes?.admins && Array.isArray(adminsRes.admins)) {
        setDelegatedAdmins(adminsRes.admins);
      }
      if (logsRes?.logs && Array.isArray(logsRes.logs)) {
        setBotAuditLogs(logsRes.logs);
      }
    } catch (e) {
      console.warn("Failed to load bot dashboard data:", e);
    } finally {
  ...
  React.useEffect(() => {
    fetchBotDashboardData();
    const botInterval = setInterval(fetchBotDashboardData, 20000);
    return () => clearInterval(botInterval);
  }, [fetchBotDashboardData]);

  const handleTriggerFaceReauth = async (phone: string, name: string) => {
    try {
      toast.loading(`Issuing Biometric Challenge to ${name || 'Staff'}...`, { id: "dash-face" });
      ...
      if (result.success) {
        toast.success(`Face verification challenge sent to ${name}'s WhatsApp!`, { id: "dash-face" });
        fetchBotDashboardData();
      } else {
        toast.error(result.error || "Failed to issue challenge.", { id: "dash-face" });
      }
    } catch (err: any) {
      toast.error(err.message || "Network error.", { id: "dash-face" });
    }
  };

  const handleDownloadManual = () => {
    try {
      generateBotUserManualPDF({
        campusName: data?.settings?.campus_name || "Superior Group of Colleges Jahanian",
        principalName: "Executive Principal Office",
        contactNumber: data?.settings?.contact_number || "0300-1234567",
      });
      toast.success("AI Executive Bot Operations Manual PDF downloaded!");
    } catch (err: any) {
  ...
  const activeIncomes = useMemo(() => {
    ...
    return data.incomes.filter((inc: any) => {
      ...
      const isStudentFee =
        feeTypeStr.includes("admission") ||
        feeTypeStr.includes("tuition") ||
        feeTypeStr.includes("installment") ||
        feeTypeStr.includes("semester") ||
        feeTypeStr.includes("exam fee");
    ...
    <div className="space-y-8 pb-12 relative">
      ...
      <div className="relative overflow-hidden bg-superior-teal rounded-[2rem] p-6 lg:p-8 border border-superior-teal/20 shadow-2xl shadow-superior-teal/20 z-10">
        ...
        <div className="relative z-20 mt-6 pt-6 border-t border-white/10 flex flex-wrap gap-2 w-full">
          {[
            ...
            { id: "ai_executive", label: "Executive Bot Console", icon: Smartphone },
      ...
      {activeDashboardTab === "overview" && (
        <>
          ...
          {/* AI Executive WhatsApp Bot & Staff Delegation Live Hub */}
          <div className="relative z-10 bg-gradient-to-br from-slate-900 via-slate-900 to-teal-950 rounded-3xl p-6 border border-teal-500/30 shadow-2xl text-white overflow-hidden">
            {/* Ambient glow */}
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-6 border-b border-white/10">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-teal-500 to-emerald-600 flex items-center justify-center shadow-lg shadow-teal-500/20 text-white shrink-0">
                  <Smartphone size={24} />
                </div>
                <div>
                  <div className="flex flex-wrap items-center gap-2 mb-1">
                    <span className="flex h-2.5 w-2.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.9)] animate-pulse" />
                    <h3 className="text-lg md:text-xl font-black uppercase tracking-tight text-white">
                      AI Executive WhatsApp Bot & Staff Security Live Hub
                    </h3>
                  ...
                  <p className="text-xs text-white/60">
                    Real-time WhatsApp voice notes, admission slip OCR, fee deposits & staff biometric security monitoring
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2.5 self-end md:self-auto">
                <Button
                  size="sm"
                  variant="outline"
                  onClick={fetchBotDashboardData}
                  disabled={isLoadingBotData}
                  className="bg-white/5 hover:bg-white/10 text-white border-white/10 text-xs font-bold gap-1.5 rounded-xl h-9"
                >
                  <RefreshCw size={14} className={isLoadingBotData ? "animate-spin" : ""} />
                  Refresh
                </Button>
                <Button
                  size="sm"
                  ...
                  className="bg-white/5 hover:bg-white/10 text-amber-300 border-amber-500/30 text-xs font-bold gap-1.5 rounded-xl h-9"
                >
                  <Download size={14} />
                  Ops Manual PDF
                </Button>
                <Button
                  size="sm"
                  onClick={() => setActiveDashboardTab("ai_executive")}
                  className="bg-gradient-to-r from-teal-500 to-emerald-600 hover:from-teal-600 hover:to-emerald-700 text-white font-black text-xs gap-1.5 rounded-xl h-9 shadow-md shadow-teal-500/20"
                >
                  Full Bot Console
                  <ArrowRight size={14} />
                </Button>
            ...
            {/* Quick Metrics Strip */}
            <div className="relative z-10 grid grid-cols-2 sm:grid-cols-4 gap-3 py-4 border-b border-white/10">
              <div className="bg-white/5 rounded-2xl p-3 border border-white/5">
                <p className="text-[10px] font-black uppercase tracking-widest text-white/50">Delegated Staff</p>
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-2xl font-black text-white">{delegatedAdmins.length}</span>
                  <span className="text-[10px] font-bold text-emerald-400">
                    ({delegatedAdmins.filter(a => a.status === "active").length} Active)
                  </span>
                </div>
              </div>
              <div className="bg-white/5 rounded-2xl p-3 border border-white/5">
                <p className="text-[10px] font-black uppercase tracking-widest text-white/50">Face Biometrics</p>
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-2xl font-black text-emerald-300">
                    {delegatedAdmins.filter(a => a.faceSnapshotUrl).length}
                  </span>
                  <span className="text-[10px] font-bold text-white/50">
                    / {delegatedAdmins.length} Enrolled
                  </span>
                </div>
              </div>
              <div className="bg-white/5 rounded-2xl p-3 border border-white/5">
                <p className="text-[10px] font-black uppercase tracking-widest text-white/50">Voice Notes Transcribed</p>
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-2xl font-black text-purple-300">
                    {botAuditLogs.filter(l => l.messageType === "voice").length}
                  </span>
                  <span className="text-[10px] font-bold text-purple-400">Audio STT</span>
                </div>
              </div>
              <div className="bg-white/5 rounded-2xl p-3 border border-white/5">
                <p className="text-[10px] font-black uppercase tracking-widest text-white/50">Slips & Images OCR</p>
                <div className="flex items-baseline gap-2 mt-1">
                  <span className="text-2xl font-black text-teal-300">
                    {botAuditLogs.filter(l => l.messageType === "image").length}
                  </span>
                  <span className="text-[10px] font-bold text-teal-400">Vision OCR</span>
                </div>
              </div>
            </div>

            {/* Split Content: Delegated Staff Matrix (Left) & Live Multimodal Activity Stream (Right) */}
            <div className="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-6 pt-5">
              
              {/* LEFT COLUMN: Delegated Staff Security Matrix */}
              <div className="lg:col-span-5 flex flex-col space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <ShieldCheck size={16} className="text-teal-400" />
                    <h4 className="text-xs font-black uppercase tracking-wider text-white">
                      Delegated Staff Access & Biometrics
                    </h4>
                  </div>
                  <Badge variant="outline" className="text-[10px] border-white/10 text-white/70">
                    {delegatedAdmins.length} Staff
                  </Badge>
                </div>
                ...
                <div className="space-y-3 max-h-[460px] overflow-y-auto pr-1">
                  {delegatedAdmins.length === 0 ? (
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-6 text-center">
                      <ShieldCheck size={32} className="mx-auto text-white/30 mb-2" />
                      <p className="text-xs font-bold text-white/80">No Delegated Staff Yet</p>
                      <p className="text-[11px] text-white/50 mt-1 mb-4">
                        Principal can grant Admissions or Fee collection access to staff via WhatsApp Bot.
                      </p>
                      <Button
                        size="sm"
                        onClick={() => setActiveDashboardTab("ai_executive")}
                        className="bg-teal-500 hover:bg-teal-600 text-white font-bold text-xs rounded-xl"
                      >
                        + Delegate New Staff
                      </Button>
                    </div>
                    ...
                    delegatedAdmins.map((admin: any) => {
                      const hasFace = Boolean(admin.faceSnapshotUrl);
                      const isPendingReauth = Boolean(admin.requiresFaceReauth);

                      return (
                        <div
                          key={admin.id || admin.phone}
                          className="bg-white/5 hover:bg-white/[0.08] transition-all border border-white/10 rounded-2xl p-4 flex flex-col gap-3 relative overflow-hidden"
                        >
                          <div className="flex items-start gap-3">
                            {/* Circular Face Snapshot or Avatar */}
                            <div className="relative shrink-0">
                              {hasFace ? (
                                <div
                                  onClick={() => setPreviewModalFace({
                                    isOpen: true,
                                    name: admin.name || "Staff Member",
                                    url: admin.faceSnapshotUrl,
                                    phone: admin.phone
                                  })}
                                  className="w-12 h-12 rounded-full ring-2 ring-emerald-500/60 overflow-hidden cursor-pointer hover:scale-105 transition-transform bg-slate-800 shadow-md group relative"
                                  title="Click to view Biometric Face Photo"
                                >
                                  <img
                                ...
                                <div className="w-12 h-12 rounded-full ring-2 ring-amber-500/40 bg-amber-500/10 flex items-center justify-center text-amber-300 font-bold text-xs">
                                  {(admin.name || "S").slice(0, 2).toUpperCase()}
                                </div>
                              )}
                              {hasFace && (
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center justify-between gap-1">
                                ...
                                >
                                  {admin.status === "active" ? "Active" : admin.status === "pending_security" ? "Setup Pending" : admin.status}
                                </Badge>
                              </div>
                              <p className="text-[11px] font-mono text-white/60 truncate">
                                📞 +{admin.phone} {admin.cnic ? `• CNIC: ${admin.cnic}` : ""}
                              </p>

                              {/* Permissions Badges */}
                              <div className="flex flex-wrap gap-1 mt-1.5">
                                {(admin.rolePermissions || ["admissions"]).map((perm: string) => (
                                  <span
                                    key={perm}
                                    className="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md bg-white/10 text-white/80 border border-white/5"
                                  >
                                    {perm}
                                  </span>
                                ))}
                          ...
                          <div className="pt-2 border-t border-white/5 flex items-center justify-between text-[10px]">
                            <div className="flex items-center gap-3">
                              {/* 5-Digit PIN */}
                              <div className="flex items-center gap-1 text-white/70" title="Security PIN">
                                <Lock size={12} className="text-amber-400" />
                                <span className="font-mono">
                                  PIN: {admin.pinLast4 ? `••••${admin.pinLast4}` : "Pending"}
                                </span>
                              </div>
                              {/* Face Biometric Status */}
                              <div className="flex items-center gap-1">
                                <Camera size={12} className={hasFace ? "text-emerald-400" : "text-amber-400"} />
                                <span className={hasFace ? "text-emerald-300" : "text-amber-300/80"}>
                                  {hasFace ? (isPendingReauth ? "⚠️ Re-Auth Required" : "Face Enrolled") : "Pending Face ID"}
                                </span>
                              </div>
                            </div>

                            {/* Re-Auth Challenge Button */}
                            {admin.status === "active" && (
                              <button
                                onClick={() => handleTriggerFaceReauth(admin.phone, admin.name)}
                                className="text-[9px] font-bold text-teal-300 hover:text-teal-200 bg-teal-500/20 hover:bg-teal-500/30 px-2 py-1 rounded-lg border border-teal-500/30 transition-colors"
                              >
                                Re-Challenge Face
                              </button>
                            )}
              ...
              {/* RIGHT COLUMN: Real-Time Multimodal Activity Stream */}
              <div className="lg:col-span-7 flex flex-col space-y-3">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                  <div className="flex items-center gap-2">
                    <Volume2 size={16} className="text-purple-400" />
                    <h4 className="text-xs font-black uppercase tracking-wider text-white">
                      Live Multimodal Activity Stream (Voice, Slips & Transactions)
                    </h4>
                  </div>
                  {/* Filter chips */}
                  <div className="flex items-center gap-1 bg-white/5 p-1 rounded-xl border border-white/10 text-[10px]">
                    {(["all", "voice", "image", "admin"] as const).map((filter) => (
                      <button
                        key={filter}
                        onClick={() => setOverviewAuditFilter(filter)}
                        className={cn(
                          "px-2.5 py-1 rounded-lg font-bold uppercase transition-all",
                          overviewAuditFilter === filter
                            ? "bg-teal-500 text-white shadow-sm"
                            : "text-white/60 hover:text-white"
                        )}
                      >
                        {filter === "all" ? "All" : filter === "voice" ? "Voice STT" : filter === "image" ? "Images/Slips" : "Admins"}
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div className="space-y-2.5 max-h-[460px] overflow-y-auto pr-1">
                  {botAuditLogs
                    .filter((log: any) => {
                      if (overviewAuditFilter === "voice") return log.messageType === "voice";
                      if (overviewAuditFilter === "image") return log.messageType === "image";
                      if (overviewAuditFilter === "admin") return log.actionType?.includes("admin") || log.actionType?.includes("face");
                      return true;
                    })
                    .slice(0, 20)
                    .map((log: any) => {
                      const isVoice = log.messageType === "voice";
                      const isImage = log.messageType === "image";
                      const isFee = log.actionType === "fee_recorded";
                      const isAdm = log.actionType === "admission_created";
                      const isAdminSetup = log.actionType === "admin_verified" || log.actionType === "face_enrolled";

                      return (
                        <div
                          key={log.id}
                          onClick={() => setSelectedAuditLog(log)}
                          className="bg-white/5 hover:bg-white/[0.08] transition-all border border-white/10 rounded-2xl p-3.5 flex items-start gap-3 cursor-pointer group"
                        >
                          {/* Event Icon */}
                          <div
                            className={cn(
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2 mb-1">
                              <div className="flex items-center gap-1.5 truncate">
                                ...
                                <Badge className="text-[8px] font-black uppercase px-1.5 py-0 bg-white/10 text-white/70 border-white/5">
                            ...
                            {log.transcript && (
                              <div className="mt-1 bg-black/20 rounded-xl p-2 border border-white/5">
                            ...
                            {isImage && log.mediaUrl && (
                              <div className="mt-2 flex items-center gap-2">
                                ...
                                <span className="text-[10px] text-teal-300 font-bold">
                                  Captured Document / Biometric Frame
                                </span>
                              </div>
                            )}
                            ...
                            <div className="mt-2 flex items-center gap-2 text-[10px]">
                              <span
                                className={cn(
                                  "font-bold uppercase tracking-wider px-2 py-0.5 rounded-md",
                                  log.status === "success"
                                    ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30"
                              ...
                              {log.verificationLevel && (
                                <span className="text-white/50 font-mono text-[9px]">
                                  Level: {log.verificationLevel.replace(/_/g, " ")}
                                </span>
                              )}
                            </div>
                          </div>
                  ...
                  {botAuditLogs.length === 0 && (
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-8 text-center">
                      <Volume2 size={32} className="mx-auto text-white/30 mb-2" />
                      <p className="text-xs font-bold text-white/80">Awaiting Bot Activity</p>
                      <p className="text-[11px] text-white/50 mt-1">
                        When users or staff send voice notes, images, admission forms, or fee verifications on WhatsApp, they will stream here in real-time.
                      </p>
                    </div>
                  )}
                </div>
      ...
      {activeDashboardTab === "ai_executive" && (
        <motion.div
          initial={{ opacity: 0, y: 15 }}
          animate={{ opacity: 1, y: 0 }}
      ...
      {/* Biometric Face Snapshot High-Res Preview Modal */}
      {previewModalFace?.isOpen && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-6 max-w-md w-full text-white shadow-2xl relative">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h4 className="font-black text-base">{previewModalFace.name}</h4>
                <p className="text-xs text-white/60 font-mono">
                  {previewModalFace.phone ? `+${previewModalFace.phone} • ` : ""}Biometric Face ID
                </p>
              </div>
              <button
                onClick={() => setPreviewModalFace(null)}
                className="w-8 h-8 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white transition-colors"
              >
                <X size={16} />
              </button>
            </div>
            <div className="rounded-2xl overflow-hidden border-2 border-emerald-500/40 bg-black aspect-square flex items-center justify-center">
              <img
                src={previewModalFace.url}
                alt={previewModalFace.name}
                className="w-full h-full object-cover"
              />
            </div>
            <p className="text-[11px] text-white/50 text-center mt-3">
              Enrolled via phone camera selfie through WhatsApp AI Security Gateway.
            </p>
          </div>
        </div>
      )}
      ...
      {selectedAuditLog && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-white/10 rounded-3xl p-6 max-w-lg w-full text-white shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/10">
              <div>
                <h4 className="font-black text-base uppercase tracking-tight">WhatsApp Bot Event Inspection</h4>
                <p className="text-xs text-white/60 font-mono">Log ID: {selectedAuditLog.id}</p>
              </div>
              <button
            ...
            <div className="space-y-3 text-xs">
              <div className="grid grid-cols-2 gap-3 bg-white/5 p-3 rounded-2xl border border-white/5">
                <div>
                  <span className="text-white/40 block text-[10px] uppercase font-bold">Sender</span>
                  <span className="font-bold text-white">{selectedAuditLog.senderName || selectedAuditLog.senderPhone}</span>
                </div>
                <div>
                  <span className="text-white/40 block text-[10px] uppercase font-bold">Action Type</span>
                  <span className="font-bold text-teal-300">{selectedAuditLog.actionType}</span>
                </div>
                <div>
                  <span className="text-white/40 block text-[10px] uppercase font-bold">Message Format</span>
                  <span className="font-bold text-purple-300 uppercase">{selectedAuditLog.messageType}</span>
                </div>
                <div>
                  <span className="text-white/40 block text-[10px] uppercase font-bold">Verification Level</span>
                  <span className="font-bold text-emerald-300">{selectedAuditLog.verificationLevel}</span>
                </div>
              </div>

              {selectedAuditLog.transcript && (
                <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                  <span className="text-white/40 block text-[10px] uppercase font-bold mb-1">
                    Speech-to-Text / Transcribed Note
                  </span>
              ...
              {selectedAuditLog.mediaUrl && (
                <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                  <span className="text-white/40 block text-[10px] uppercase font-bold mb-2">
                    Captured Media Snapshot
                  </span>
                  <div className="max-h-64 rounded-xl overflow-hidden border border-white/10 flex items-center justify-center bg-black">
                    <img
                      src={selectedAuditLog.mediaUrl}
                      alt="Event Media"
              ...
              {selectedAuditLog.details && (
                <div className="bg-white/5 p-3 rounded-2xl border border-white/5">
                  ...
                  <pre className="text-[11px] font-mono text-white/80 bg-black/40 p-2.5 rounded-xl overflow-x-auto">
